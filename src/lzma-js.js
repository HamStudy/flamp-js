...
var LZMA = function () {
   ...
   * @param {number} mode        Mode to compress with 1-9, 1 is fastest 9 slowest, slower is probably better compression
   * @param {(res: string, err?: Error) => void} on_finish     callback function called when complete
   * @param {(percent: number) => void} on_progress   callback for progress
   * 
   * @returns {string | Uint8Array}   returns a string if it appears to be valid UTF8, otherwise a Uint8Array
   */
   ...
   * @param {Uint8Array} byte_arr Typed array of bytes to decode
   * @param {(result: string | Uint8Array) => void} on_finish callback to call on finish
   * @param {(percent: number) => void} on_progress callback to call on progress
   */
